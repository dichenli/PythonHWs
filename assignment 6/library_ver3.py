...
class Patron(object):
    ...
    def __init__(self, name):
        ...
        self.name = name
        self.books = set({}) #Assumption here: Each patron can only take one copy of the same book!
    ...
    def __eq__(self, other):
        ...
        return type(other) == type(self) and self.get_name() == other.get_name()
    ...
    def __str__(self):
        ...
        for book in self.set_of_books:
            ...
            if due_date < current_date:
                message = message + "OVERDUE: " + book.get_title() + ", by " + book.get_author() + " (" + " was due on day " + str(book.get_due_date()) + ")\n"
            elif due_date == current_date:
                message = message + "DUE: " + book.get_title() + ", by " + book.get_author() + " (due today)\n"
            else:
                message = message + "DUE: " + book.get_title() + ", by " + book.get_author() + " (" + " is due on day " + str(book.get_due_date()) + ")\n"
        return message
...
class Library(object):
    ...
    def list_overdue_books(self):
        ...
        for patron in self.patrons:
            message += self.list_overdue_for_patron(patron)
        if message == '':
            message = "No overdue book was found!"
        self.talk(message)
    ...
    def issue_card(self, name_of_patron):
        ...
        if not self.is_open:
            ...
            return
        if name_of_patron == "":
            self.talk("Error! The name can't be empty!")
            return
    ...
    def serve(self, name_of_patron):
        ...
        for patron in self.patrons:
            if patron.get_name() == name_of_patron:
                ...
                self.talk("Now helping %s.\n%s has these books:" % (name_of_patron, name_of_patron))
                self.talk(self.create_numbered_list(self.patron_being_served.get_books()))
                return
    ...
    def check_in(self, *book_numbers):
        ...
        for number in book_numbers:
            if number in self.found_books and self.found_books[number] in self.patron_being_served.get_books():
    ...
    def search(self, string):
        ...
        for book in self.collection:
            if not book.get_due_date():
                if (book.get_title().lower()).find(string.lower())>= 0 or (book.get_author().lower()).find(string.lower()) >= 0: 
                    found_books.append(book)
        self.talk(self.create_numbered_list(found_books))
...
##            self.talk("Sorry, no book was found!")
    ...
    def check_out(self, *book_numbers):
        ...
        if not self.is_open:
            self.talk("The library is closed now. Please wait for openning before any operations.\n")
            return 
...
##lib.open()
##lib.search("Thomas")
##print lib.found_books
##lib.create_numbered_list([lib.collection[0], lib.collection[3]])
##print lib.found_books
##print lib.found_books[1].get_title()
##lib.issue_card("Amy")
##print lib.response
##lib.close()
##lib.open()
##print str(lib.patron_being_served)
##lib = Library()
##lib.open()
##lib.issue_card("Amy")
###lib.search("2")
...
####
####lib.response = ''
####lib.list_overdue_for_patron(lib.patron_being_served)
####
####for i in range(1, 10):
####    lib.close()
####    lib.open()
####lib.response = ''
####lib.serve("Amy")
####print lib.list_overdue_for_patron(lib.patron_being_served)
####lib.open()
####lib.issue_card("Amy")
...
##lib.search("Thomas")
##lib.check_out(1, 2) #check out two books to Bilibili
