...
class Library(object):
    ...
    def open(self):
        ...
        if self.is_open == True:
            ...
            self.is_open = True
            self.talk("The library is open now! Today is day %d" % calendar.get_date())
    ...
    def serve(self, name_of_patron):
        ...
        for pat in self.patrons:
            if pat.get_name() == name_of_patron:
                self.patron_being_served = pat
                self.talk("%s is now being served." % name_of_patron)
                return
        print "The patron was not found! This person is not in patrons list"
...
##                self.collection.remove(book)
    ...
    def search(self, string):
        ...
        for book in self.collection:
            if book.get_title() == string.strip or book.get_author == string.strip:
                self.found_books.append(book)
        if len(self.found_books) == 0:
            self.talk("Sorry, no book was found!")
        else:
            self.talk(create_numbered_list(self.found_books))
        return self.found_books
